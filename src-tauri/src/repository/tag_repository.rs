...
use crate::models::{Tag, CreateTag, UpdateTag, TagCategory};
...
mod tests {
    ...
    async fn test_get_tag_by_id() {
        ...
        let created = TagRepository::create(&pool, CreateTag {
    ...
    async fn test_get_tag_by_name() {
        ...
        TagRepository::create(&pool, CreateTag {
        ...
        let fetched = TagRepository::get_by_name(&pool, "philadelphia").await.unwrap();
    ...
    async fn test_tag_hierarchy() {
        ...
        // Create parent tag
        let parent = TagRepository::create(&pool, CreateTag {
            name: "pasta".to_string(),
            display_name: "Pasta".to_string(),
            category: TagCategory::Ingredient,
            weekly_suggestion: Some(3),
        ...
        // Create child tag
        let child = TagRepository::create(&pool, CreateTag {
    ...
    async fn test_update_tag() {
        ...
        let updated = TagRepository::update(&pool, created.id, UpdateTag {
    ...
    async fn test_get_by_category() {
        ...
        let ingredients = TagRepository::get_by_category(&pool, TagCategory::Ingredient).await.unwrap();
        assert_eq!(ingredients.len(), 1);
        assert_eq!(ingredients[0].name, "pasta");

        let dietary = TagRepository::get_by_category(&pool, TagCategory::Dietary).await.unwrap();
    ...
    async fn test_unique_tag_name() {
        ...
        let result = TagRepository::create(&pool, CreateTag {
            name: "duplicate".to_string(),
